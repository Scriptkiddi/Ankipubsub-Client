...
class connectionHandler(object):
	...
	def __init__(self, serverUrl, username, password):
		...
		self.session = requests.Session() #Session damit login Informationen mit gespeichert werden
	...
	def push_deck(self,deck):
		...
		#Schreibe daten struktur um da die Karten eine eigene Klasse sind die nur von UserDict geerbt hat
		for note in notes:
		...
		#wenn das deck schon eine ID hat dann wird an diese ID geschickt wenn nicht an den normalen Handler
		if deck.getRemoteID():
		...
		#Führe Post aus
		deckResponse = self.session.post(url, data=json.dumps(payload), headers=headers).json()
	...
	def pull_deck(self, deckid, **kwargs):
		...
		deckResponse = self.session.get(self.url+'/pull/deck/'+deckid, params=payload)
		...
		#Erzeuge aus den Karten in dieser Antwort AnkipupSub Objecte und ersetze damit die Json Liste
		newNotes = []
	...
	def login(self):
		r = self.session.get(self.url+'/login', auth=(self.username, self.password))
