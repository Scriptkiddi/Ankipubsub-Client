...
def sync(localDeckID, serverURL, username, password, firsttime=True):
    ...
    if not localDeckToPush.getRemoteID():
        ...
        for all those who have not wanderd the path of date\
        convertion enligthment here is why we do this
        Anki sets the lokals when you select your language\
         and python is retarded and think oh hey lets
        make our timestamps fit to that local so if you are\
         german strftime('%a') returns Di(Dienstag) instead of Tue(Tuesday)
        to make sure we are on the same date as my server is we drop every\
        local settings for the date conversion
